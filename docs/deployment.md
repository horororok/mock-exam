...
`/admin`(시험 등록·삭제)은 **계정 기반**이다. `ADMIN_EMAILS`(쉼표 구분)에 등록된 이메일로
로그인한 사용자만 관리자 권한을 갖는다. 별도 관리자 계정은 없고, 일반 회원가입 계정 + 화이트리스트다.
미설정 시: 개발 환경은 열림(편의), **프로덕션은 잠금**(아무도 관리자 아님).

```sh
# 로컬: .dev.vars 에 추가
ADMIN_EMAILS='you@example.com,teacher@example.com'

# 프로덕션: Cloudflare secret 으로 등록 (또는 wrangler.jsonc vars)
pnpm wrangler secret put ADMIN_EMAILS
```

설정 후, 해당 이메일로 회원가입/로그인하면 `/admin`·`/admin/upload`가 열린다(세션 쿠키로 판별).

## 배포 전 체크리스트

- [ ] **`ADMIN_EMAILS` 설정** (로컬 `.dev.vars` + 프로덕션 `wrangler secret put`) — /admin 보호. 미설정 시 프로덕션 /admin은 잠김
- [ ] 원격 D1 마이그레이션 적용 완료

...
> ⚠️ `/admin`·`/admin/upload`은 **관리자 계정만** 접근할 수 있습니다. `ADMIN_EMAILS`(쉼표 구분)에
> 등록된 이메일로 **로그인**한 사용자가 관리자입니다(세션 쿠키로 판별). 미설정 시: 개발환경은 열림,
> 프로덕션은 잠김. 설정 방법은 [deployment.md](./deployment.md).
